...
static unsigned char sd_pwr_sts = 0;
static int msmsdcc_setup_vreg(struct msmsdcc_host *host, bool enable)
{
	...
	if (is_sd_platform(host->plat) && (sd_pwr_sts ^ enable)) {
		sd_pwr_sts = enable;
		mdelay(5);
		if (enable)
