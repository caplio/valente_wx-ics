...
int adreno_dump_and_recover(struct kgsl_device *device)
{
	...
	struct adreno_recovery_data rec_data;
	static int recover_counter = 0;
	unsigned int rbbm_status ;
	static unsigned int pre_rbbm_status = false;
	...
	if (device->state == KGSL_STATE_DUMP_AND_RECOVER) {
		...
		if(rbbm_status == pre_rbbm_status)
			recover_counter++;
		else
			recover_counter = 0;
		pre_rbbm_status = rbbm_status;

		if(recover_counter == RECOVER_MAX_COUNT)
		...
		/* HTC: Consider to trigger system panic for GPU Hang problem */
		if (!device->snapshot_no_panic) {
			if(recover_counter != RECOVER_MAX_COUNT ){
				if(recover_counter == 1)
					KGSL_DRV_ERR(device, "detect GPU hang and try to recover %d trial\n",
					recover_counter);
				else
					KGSL_DRV_ERR(device, "detect GPU hang and try(first trial failed) to recover %d trial\n",
					recover_counter);
			...
			KGSL_DRV_ERR(device, "detect GPU but no_panic is %d, skip panic..\n",
