...
static int msm_hsl_irda_enable_set(void *data, u64 val)
{
	...
	if (val) {
		...
		gpio_set_value(GPIO_IRDA_PWR, 0);
		usleep(100);
		gpio_set_value(GPIO_IRDA_PWR, 1);
		usleep(500);
		gpio_set_value(GPIO_IRDA_PWR, 0);
		usleep(500);
...
static irqreturn_t msm_hsl_irq(int irq, void *dev_id)
{
	...
	if (misr & (UARTDM_ISR_RXSTALE_BMSK | UARTDM_ISR_RXLEV_BMSK)) {
		handle_rx(port, misr);
		if (misr & (UARTDM_ISR_RXSTALE_BMSK)) {
			/*pr_info("%s () UARTDM_ISR_RXSTALE_BMSK:port->line %d, ir\n", __func__, port->line);*/
...
static void msm_hsl_set_baud_rate(struct uart_port *port, unsigned int baud)
{
	...
	if (port->line == 2) {
		...
		if (force_baud_1 == 96)
			...
			baud = 115200;
		/*pr_info("%s: irda change_baud is %d,\n", __func__, baud);*/
	}
...
void enable_irda(int irda_en)
{
	...
	pr_info("%s(): irda msm_irda_port->irda_shutdown_gpio(%d), irda_en %d\n",
		 __func__, msm_irda_port->irda_shutdown_gpio, irda_en);
	...
	if (irda_en) {
		...
		gpio_set_value(msm_irda_port->irda_shutdown_gpio, 0);
		usleep(100);
		gpio_set_value(msm_irda_port->irda_shutdown_gpio, 1);
		usleep(500);
		gpio_set_value(msm_irda_port->irda_shutdown_gpio, 0);
		usleep(500);
...
static int __devinit msm_serial_hsl_probe_irda(struct platform_device *pdev)
{
	...
	printk(KERN_INFO "msm_serial_irda: irda_shutdown_gpio #%d\n", pdata->irda_shutdown_gpio);
	msm_hsl_port = UART_TO_MSM(port);
	msm_hsl_port->irda_shutdown_gpio = pdata->irda_shutdown_gpio;
