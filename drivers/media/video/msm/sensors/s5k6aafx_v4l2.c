...
static struct s5k6aafx_format s5k6aafx_subdev_info[] = {
	{
...
static int s5k6aafx_enum_fmt(struct v4l2_subdev *sd, unsigned int index,
			   enum v4l2_mbus_pixelcode *code)
{
	printk(KERN_DEBUG "Index is %d\n", index);
	pr_info("%s", __func__);
	if ((unsigned int)index >= ARRAY_SIZE(s5k6aafx_subdev_info))
		return -EINVAL;

	*code = s5k6aafx_subdev_info[index].code;
	return 0;
}

static struct v4l2_subdev_core_ops  s5k6aafx_subdev_core_ops;
static struct v4l2_subdev_video_ops  s5k6aafx_subdev_video_ops = {
	.enum_mbus_fmt =  s5k6aafx_enum_fmt,
};
...
static int s5k6aafx_set_qtr_size_mode(enum qtr_size_mode qtr_size_mode_value)
{
	...
	if (sinfo->csi_if) {
		...
		case NORMAL_QTR_SIZE_MODE:
			...
			rc = s5k6aafx_i2c_write_table(&s5k6aafx_regs.prev_mode_switch_VGA[0],s5k6aafx_regs.prev_mode_switch_VGA_size);
			mdelay(150);
...
static int s5k6aafx_set_sensor_mode(int mode, enum qtr_size_mode qtr_size_mode_value)
{
	...
	struct msm_camera_sensor_info *sinfo = s5k6aafx_pdev->dev.platform_data;
	/*MIPI setting*/
	struct msm_camera_csid_params s5k6aafx_csid_params;
	struct msm_camera_csiphy_params s5k6aafx_csiphy_params;
	struct msm_camera_csid_vc_cfg s5k6aafx_vccfg[] = {
	...
	if (config_csi == 0) {
		if (sinfo->csi_if) {
			...
			v4l2_subdev_notify(s5k6aafx_ctrl->sensor_dev,
				NOTIFY_ISPIF_STREAM, (void *)ISPIF_STREAM(
				PIX0, ISPIF_OFF_IMMEDIATELY));


			s5k6aafx_csid_params.lane_cnt = 1;
			s5k6aafx_csid_params.lane_assign = 0xe4;
			s5k6aafx_csid_params.lut_params.num_cid =
				ARRAY_SIZE(s5k6aafx_vccfg);
			s5k6aafx_csid_params.lut_params.vc_cfg =
				&s5k6aafx_vccfg[0];
			s5k6aafx_csiphy_params.lane_cnt = 1;
			s5k6aafx_csiphy_params.settle_cnt = 20;
			v4l2_subdev_notify(s5k6aafx_ctrl->sensor_dev,
					NOTIFY_CSID_CFG, &s5k6aafx_csid_params);
			v4l2_subdev_notify(s5k6aafx_ctrl->sensor_dev,
			...
			msleep(1);
			v4l2_subdev_notify(s5k6aafx_ctrl->sensor_dev,
					NOTIFY_CSIPHY_CFG, &s5k6aafx_csiphy_params);
			dsb();
			config_csi = 1;
...
/*HTC_END*/
	...
	mdelay(2);
	if (rc<0)
		pr_err("%s, s5k6aafx mclk disable failed", __func__);

	if (!sdata->power_down_disable)
		rc = s5k6aafx_vreg_disable(s5k6aafx_pdev);
	if (rc<0)
		pr_err("%s, s5k6aafx vreg disable failed", __func__);
...
static int s5k6aafx_sensor_probe(const struct msm_camera_sensor_info *info,
...

{
	...
	if (!sdata->power_down_disable)
		rc = s5k6aafx_vreg_enable(s5k6aafx_pdev);
	if (rc<0)
	...
	mdelay(1);
	rc = s5k6aafx_gpio_pull(info->sensor_reset, 1);
	if (rc<0)
		pr_err("%s, gpio  %d enable failed", __func__, info->sensor_reset);

    rc = s5k6aafx_sensor_read_id(info);
	if (rc < 0 || s5k6aafx_client->addr != 0x5a >> 1)
		goto read_id_fail;
	else {
...
static int s5k6aafx_sensor_probe_cb(const struct msm_camera_sensor_info *info,
	struct v4l2_subdev *sdev, struct msm_sensor_ctrl *s)
{
	...
	if (!s5k6aafx_ctrl) {
		pr_err("s5k6aafx_sensor_probe failed!\n");
		return -ENOMEM;
	}

	rc = s5k6aafx_sensor_probe(info, s);
	if (rc < 0)
		return rc;
	...
	if (sdev) {
		v4l2_i2c_subdev_init(sdev,s5k6aafx_client,
						&s5k6aafx_subdev_ops);
		s5k6aafx_ctrl->sensor_dev = sdev;
	}
...
{
	s5k6aafx_pdev = pdev;
	return msm_sensor_register(pdev, s5k6aafx_sensor_probe_cb);
}
