...
static int spk_set_ctl(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol) {
	int value = ucontrol->value.integer.value[0];
	pr_info("%s: value = %d status = %d\n", __func__, value, msm8960_ext_bottom_spk_pamp);
	if (value == 1 && msm8960_ext_bottom_spk_pamp)
		gpio_direction_output(PAMP_GPIO, 1);
	else {
		gpio_direction_output(PAMP_GPIO, 0);
	}
...
static int __init valente_wx_audio_init(void)
{
	...
	rc = gpio_request((PAMP_GPIO), "amp_en");

	rc = pm8xxx_gpio_config((PAMP_GPIO),
			&param);

	if (rc < 0)
		pr_aud_err("failed to configure pmic 18 gpio\n");
